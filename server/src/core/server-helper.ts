    ...
    static registerHandlers(server: Server) {
        ...
        serverRoutes.forEach((route) => {
            if (typeof route.handler === 'string') {
                const controllerName = (route.handler as string).split('.')[0];
                const controllerMethod = (route.handler as string).split('.')[1];
                const correspondingController = serverControllers.find((a) => a.name === controllerName);
                ...
                if (correspondingController) {
                        ...
                        server.route([
                            {
                                ...route,
                                // @ts-ignore
                                handler: correspondingController[controllerMethod].bind(correspondingController),
                            },
                        ]);
                    } else {
                        Logger.error(
                    ...
                    Logger.error(`Could not find controller for ${route.handler}`);
                    process.exit(1);
                }
            } else {
                server.route(route);
